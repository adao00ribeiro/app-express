...
const itensRepository = {
    criar: (item: Item, callback: (id?: number) => void) => {
        const sql = 'INSERT INTO tb_item (nome, descricao) VALUES (?, ?)'
    ...
    lerTodos: (callback: (itens: Item[]) => void) => {
        const sql = 'SELECT * FROM tb_item'
        const params: any[] = []
        database.all(sql, params, (_err, rows) => callback(rows))
    },
    ler: (id: number, callback: (item?: Item) => void) => {
        const sql = 'SELECT * FROM tb_item WHERE id = ?'
        const params = [id]
        database.get(sql, params, (_err, row) => callback(row))
    },
    atualizar: (id: number, item: Item, callback: (notFound: boolean) => void) => {
        const sql = 'UPDATE tb_item SET nome = ?, descricao = ? WHERE id = ?'
    ...
    apagar: (id: number, callback: (notFound: boolean) => void) => {
        const sql = 'DELETE FROM tb_item WHERE id = ?'
        const params = [id]
        database.run(sql, params, function(_err) {
            callback(this.changes === 0)
        })
